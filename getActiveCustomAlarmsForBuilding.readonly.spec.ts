...
import { getActiveCustomAlarmsForBuilding } from '@/tests/playwright/tests/api/abound-alarm-gql/alarms/customAlarms.step';
import { getCustomerForEnv } from '@/tests/playwright/framework/entities/Customer';

test.describe(
  'active custom alarms for building',
  {
    tag: ['@regression', '@api'],
  },
  () => {
    ...
    test('get active custom alarms for building', async ({ gqlRunnerByGlobalAdmin }) => {
      const alarms = await getActiveCustomAlarmsForBuilding(gqlRunnerByGlobalAdmin, building.id, customerId);
